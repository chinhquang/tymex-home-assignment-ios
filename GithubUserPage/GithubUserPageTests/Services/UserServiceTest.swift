...
final class UserServiceTest: XCTestCase {
    ...
    func testFetchUserListSuccess() async throws {
        // Arrange
        let mockData = JSONLoader.loadJSON(from: "mock_users_data_success", in: Bundle(for: type(of: self)))
    ...
    func testFetchUserListEmptySuccess() async throws {
        // Arrange
        let mockData = JSONLoader.loadJSON(from: "mock_users_data_success_empty_data", in: Bundle(for: type(of: self)))
        MockURLProtocol.requestHandler = { _ in
            let response = HTTPURLResponse(url: URL(string: "https://example.com")!, statusCode: 200, httpVersion: nil, headerFields: nil)!
            return (mockData, response, nil)
